...
export const NowWatching = () => {
  const [showCleanPoster, setShowCleanPoster] = useState(true);

  useEffect(() => {
    const posterTimer = window.setTimeout(() => setShowCleanPoster(false), 3800);
        ...
        <div className="relative w-full mx-auto max-w-[400px] md:max-w-none md:h-[100vh] md:aspect-auto" style={{ aspectRatio: "9 / 16" }}>
          ...
          {showCleanPoster && (
            <img
              ...
              className="pointer-events-none absolute inset-0 h-full w-full rounded-md object-cover transition-opacity duration-500"
              loading="eager"
              fetchPriority="high"
            />
          )}
